...
import { MiniPeriodCalendar } from "@/components/mini-period-calendar";
...
export default function AddLog() {
  ...
  useEffect(() => {
    const fetchPeriodData = async () => {
      try {
        ...
        if (data.entries) setPeriodEntries(data.entries);
      } catch (error) {
  ...
  const handleFormSubmit = async (data) => {
    try {
      ...
      if (isUpdate) {
        setPeriodEntries((prev) => prev.map((entry) => (entry.id === result.entry.id ? result.entry : entry)));
      } else {
